...
def get_keras_train_input(pair_file, histogram_file):
    ...
    # np histogram
    histogram_input = np.zeros((data_count, maxQueryLength, histogramsize), dtype=np.float32)

    # topic idf
    idf_input = np.zeros((data_count, maxQueryLength, 1), dtype=np.float32)
...
def load_histogram_data(filepath):
    ...
    with open(filepath, 'r') as inputFile:
        for line in inputFile:
            ...
            # handle idfs
            #
            idfs = np.zeros((maxQueryLength, 1), np.float32)
